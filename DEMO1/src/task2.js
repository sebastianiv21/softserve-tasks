const envelopes = (env1, env2) => {
  ...
  return { status: 'failed', reason: 'Input parameters must be objects' };
  if (!env1.hasOwnProperty('a') || !env1.hasOwnProperty('b'))
  return { status: 'failed', reason: 'Envelope 1 must have a & b props' };
  if (!env2.hasOwnProperty('c') || !env2.hasOwnProperty('d'))
  return { status: 'failed', reason: 'Envelope 2 must have c & d props' };
  if (env1.a < 0 || env1.b < 0 || env2.c < 0 || env2.d < 0)
  return { status: 'failed', reason: 'Envelope\'s dimensions must be greater than zero' };
